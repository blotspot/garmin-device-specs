...
def get_device_ids() -> set:
    ...
    device_ids = {
        a["href"].strip("/").split("/")[-1].replace("-", "_") for a in device_links
    }
...
def enrich_with_api_levels(all_devices_data: dict) -> dict:
    ...
    name_to_id_map = {
        ...
        for device_id, device in all_devices_data.items()
        if device.get("Name")
    }
    ...
    for device_name, api_level in api_levels_by_name.items():
        ...
        if device_id:
            ...
            print(
                f"API Level mapping: No device reference found for '{device_name}'. Skipping."
            )
...
def main():
    ...
    if new_ids:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            results = list(
                tqdm(
                    executor.map(parse_device_details, new_ids),
                    total=len(new_ids),
                    desc="Fetching new devices",
                )
            )
